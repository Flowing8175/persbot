...
class SessionManager:
    ...
    def get_or_create(
        ...
        # Always start with few-shot examples to guide model behavior
        history = get_few_shot_examples_as_content()

        # Append user's conversation history if available
        user_history = self._build_chat_history(user_id)
        if user_history:
            history.extend(user_history)

        # Create new chat with context (few-shot examples + conversation history)
    ...
    def _build_chat_history(self, user_id: str, limit: int = 20) -> list:
        """Build chat history from database as Content objects.
        ...
        Returns:
            Chat history as Content objects, or empty list if no history exists
        """
        try:
            ...
            # Format user's conversation history as Content objects for Gemini API
            formatted_history = []
